...
bool ArbolB::encontrarNodo(nodoB *Act,string &cl, int *pos){
	(*pos)=0;
	int t;
	//while((*pos)<Act->clavesUsadas && Act->claves[(*pos)]<cl)
	while((*pos)<Act->clavesUsadas && ( t= compara(Act->claves[*pos],cl) )<0)
		(*pos)++;
	
	return t==0;
}
string* ArbolB::busca(nodoB* nodo,string &cadena){
	if(nodo){
		int i=nodo->clavesUsadas/2,n,m=0;
		while(0<=i && i< nodo->clavesUsadas){

			if(!(n=compara(nodo->claves[i],cadena)))
				return &nodo->claves[i];
			if(!(n+m))break;
			m=n;
			i=i-n;
...
nodoB* ArbolB::encontrar(string &cl,int *pos,nodoB *Nodo,bool r){
	if(r) Nodo=raiz;
	if(!Nodo)
		return 0;
	else{
...
void ArbolB::insertar(string &cl){
	int up;
	string clMedian;
	nodoB *pnew,*nd;
	push(raiz,cl,&up,&clMedian,&nd);
...
void ArbolB::push(nodoB *Act,string &cl,int* Up,string * clMedian,nodoB ** newnode){
...
void ArbolB::putLeaf(nodoB *Act,string cl,nodoB* rd,int k){
	int i;
	for(i=Act->clavesUsadas-1;i>=k;i--){		
		Act->claves[i+1]=Act->claves[i];
		Act->hijos[i+2]=Act->hijos[i+1];
...
void ArbolB::dividirNodo(nodoB *Act,string &cl,nodoB *rd,int k,string *clMedian,nodoB **newnode){
	int i,posMdna;
	posMdna=(k<=orden/2)?orden/2:orden/2+1;	
